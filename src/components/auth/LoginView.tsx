...
import { ShieldCheck, User, Lock, ArrowRight, Database, Wifi, WifiOff } from "lucide-react";
  ...
  const handleLogin = async () => {
    if (!db) {
      toast({
        ...
        description: "Firebase connection is not established. Please refresh.",
      });
      return;
    }
    ...
    setLoading(true);
    try {
      // Explicitly check the document in the 'users' collection
      ...
      if (userSnap.exists()) {
        ...
        // Check password (case-sensitive as per storage)
        ...
        toast({
          ...
          description: `No ID found for [${cleanCode}]. Check for typos.`,
        });
      }
    } catch (error: any) {
      ...
      toast({
        ...
        description: error.message || "Failed to communicate with database.",
      });
    } finally {
      setLoading(false);
    }
        ...
        <div className="text-center space-y-2">
          ...
          <div className="flex flex-col items-center">
            ...
            >
              ...
              {db ? "Server Online" : "Server Offline"}
            </Badge>
          </div>
